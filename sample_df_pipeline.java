...
public class SamplePipeline {
	...
	public static void main(String[] args) throws IOException {
		...
		//String modelTable = "samplecategory-test-msaha";
		...
		PCollection<TableRow> sampleCategory = p.apply(
				BigQueryIO.Read
					.named("ReadOfferCategory")
					.fromQuery(getQuery(queryFileName))
					.usingStandardSql());
		
		PCollection<KV<String, TableRow>> sampleCategoryKeyAndRows = sampleCategory.apply(
	            ParDo.named("ConvertTableRowToKeyTableRow").of(new ConvertTableRowToKeyTableRow()
	                )
	            );

	    PCollection<KV<String, Iterable<TableRow>>> groupedSampleCategory =  sampleCategoryKeyAndRows.apply(GroupByKey.<String, TableRow>create());

		
		PCollection<Mutation> offerCategoryBTPut = groupedSampleCategory.apply(
				ParDo.named("SampleTableRowToMutationDoFn").of(new SampleTableRowToMutationDoFn()));
